    ...
    def styleize(self, style):
        for k in style:
            if k not in self.ignore and k not in globalignore:
                ...
                if isinstance(k, str):
                    try:
                        self.style[k] = compile(k,mode="eval")
                    except:
...
code = type(compile("print('hi')","Code",mode="eval"))

class widget(Styleizor):

    def calc(self, input="", lglobals=None):
        if isinstance(input, str) or isinstance(input,code):
            if lglobals is None:
...
class listWidget(widget):
    ...
    def hscrollredraw(self, surfacein):
        ...
        bx = self.barx
        by = self.bary
        ofx = 0 if bx.w == self.w else (scx.rx / (self.w - bx.w)) * (
            0 if (self.hfitchildren() - self.w) < 0 else (self.hfitchildren() - self.w))
        ofy = (scy.ry / (self.h - by.h)) * (self.vlargest() - self.h)
        drawx -= ofx
        drawy -= ofy
        for x in self.children:
    ...
    def vscrollredraw(self, surfacein):
        ...
        # drawy = 0
        ofx = (scx.rx / (self.w - bx.w)) * (self.hlargest() - self.w)
        ofy = 0 if by.h == self.h else (scy.ry / (self.h - by.h)) * (
            0 if (self.vfitchildren() - self.h) < 0 else (self.vfitchildren() - self.h))
